...
final class WpdbReportQueue implements ReportQueue
{
    ...
    {
        // push() runs on the decision path, so a missing/broken queue table must degrade silently
        // (no wpdb error echo — that is a fingerprint tell). WpdbSilencer restores the caller's prior
        // suppression in finally, so the nested enforceCap() -> count() wrap restores correctly too.
    ...
    {
        $rows = WpdbSilencer::silently($this->wpdb, function () use ($limit) {
    ...
    {
        WpdbSilencer::silently($this->wpdb, function () use ($id) {
    ...
    {
        return (int) WpdbSilencer::silently($this->wpdb, function () {
            return $this->wpdb->get_var("SELECT COUNT(*) FROM {$this->queueTable}");
        });
    }
    ...
    public function recentlyReported(string $key, int $withinHours)
    {
        ...
        $found = WpdbSilencer::silently($this->wpdb, function () use ($key, $cutoff) {
    ...
    {
        WpdbSilencer::silently($this->wpdb, function () use ($key) {
            $this->wpdb->query($this->wpdb->prepare(
    ...
    {
        $today = gmdate('Y-m-d');
        return (int) WpdbSilencer::silently($this->wpdb, function () use ($today) {
            return $this->wpdb->get_var($this->wpdb->prepare(
    ...
    {
        $today = gmdate('Y-m-d');
        WpdbSilencer::silently($this->wpdb, function () use ($today) {
            $this->wpdb->query($this->wpdb->prepare(
     ...
     * Called from within push()'s suppression wrap; count() re-wraps (nested, restore-safe).
     */
